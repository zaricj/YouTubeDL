...
import threading, subprocess, os, ffmpeg, re, webbrowser
...
# Add the FFmpeg binary directory to the PATH
    ...
    def download_youtube_audio(YouTubeURL, output_path, chosen_audio_quality):
        try:
            ...
            if len(chosen_audio_quality) != 0 and values["-DOWNLOAD_FORMAT-"] == "Audio Format":
                window["-STREAM_INFO_BUTTON-"].update(disabled=True)
                window["-OUTPUT_WINDOW-"].update("Starting download and ffmpeg encoder... please wait.\n")
                yt = YouTube(YouTubeURL, use_oauth=False, allow_oauth_cache=True, on_progress_callback=progress_callback)
                ...
                window["-BUTTON_DOWNLOAD-"].update(disabled=False) # Uncomment later?
                window["-STREAM_INFO_BUTTON-"].update(disabled=False)
                window["-PBAR-"].update(0)
            else:
                window["-OUTPUT_WINDOW-"].update("ERROR: Select a Quality of Stream first.")
    ...
    def video_stream(YouTubeURL):
        try:
            ...
            window["-OUTPUT_WINDOW-"].update(f"Loading... please wait.")
            ...
            for stream in video_streams:
                window["-OUTPUT_WINDOW-"].update(f"Available Video Streams:\nResolution: {stream.resolution}, Codec: {stream.subtype}")
    ...
    def audio_stream(YouTubeURL):
        try:
            ...
            for stream in audio_streams:
                window["-OUTPUT_WINDOW-"].update(f"Available Audio Streams:\nAbr: {stream.abr}, Codec: {stream.subtype}")
    ...
    right_click_menu_input_field = ["&Right", ["&Copy", "&Paste"]] # Create right click menu for input field
    ...
    while True:
        ...
        # Append URLS to list and updating PySimpleGUI's ListBox
            ...
            if "https://www.youtube.com/watch" not in url:
                ...
                window["-OUTPUT_WINDOW-"].update("")
                yt = YouTube(url)
                window["-OUTPUT_WINDOW-"].print(f"Added YouTube Video to list: {yt.title}")
        ...
        # Downloading Video File
            ...
            if len(values["-SAVE_TO_FOLDER-"]) == 0:
                ...
                for link_in_list in urls: # For every YouTube URL in the list "urls" which is in the ListBox Element start threads and download each item from the list
                    window.perform_long_operation(lambda: download_youtube_video(link_in_list, values["-SAVE_TO_FOLDER-"], values["-QUALITY_FORMAT-"]),"-OUTPUT_WINDOW-")
        ...
        # Downloading Audio File
            ...
            if len(values["-SAVE_TO_FOLDER-"]) == 0:
                ...
                for link_in_list in urls: # For every YouTube URL in the list "urls" which is in the ListBox Element start threads and download each item from the list
                    window.perform_long_operation(lambda: download_youtube_audio(link_in_list, values["-SAVE_TO_FOLDER-"], values["-QUALITY_FORMAT-"]),"-OUTPUT_WINDOW-")
        ...
        # Get Stream Info of YouTube Link for Video Downloading
            ...
            for index in selected_indices:
                if len(urls) > 0:
                    get_video_stream_thread = threading.Thread(target=video_stream, args=(urls[index],))
                    get_video_stream_thread.start()
            else:
                window["-OUTPUT_WINDOW-"].update("ERROR: Cannot get info of a video stream. Please click a link in the list and try again.")
         ...
         # Get Stream Info of YouTube Link for Audio Downloading
            ...
            for index in selected_indices:
                if len(urls) > 0:
                    get_audio_stream_thread = threading.Thread(target=audio_stream, args=(urls[index],))
                    get_audio_stream_thread.start()
            else:
                window["-OUTPUT_WINDOW-"].update("ERROR: Cannot get info of an audio stream. Please click a link in the list and try again.")

        elif event == "-URL_REDIRECT-":
