...
def main():
        ...
        for yt_urls in youtube_urls:
            ...
            try:
                ...
                if len(chosen_resolution) != 0 and values["-DOWNLOAD_FORMAT-"] == "Video Format":
                    window["-BUTTON_DOWNLOAD-"].update(disabled=True)
                    window["-STREAM_INFO_BUTTON-"].update(disabled=True)
                    window["-OUTPUT_WINDOW-"].update("Starting download and encoding... please wait.\n")

                    yt = YouTube(yt_urls, use_oauth=False, allow_oauth_cache=True,on_progress_callback=progress_callback)
                    ...
                    if video_streams:
                        ...
                        the_video_file = os.path.join(output_path, cleared_video_filename)
        ...
        for yt_urls in youtube_urls:
            ...
            try:
                ...
                if len(chosen_audio_quality) != 0:
                    window["-STREAM_INFO_BUTTON-"].update(disabled=True)
                    window["-OUTPUT_WINDOW-"].update(f"Starting download and ffmpeg encoder for index: {counter} link... please wait.\n")
                    yt = YouTube(yt_urls, use_oauth=False, allow_oauth_cache=True,on_progress_callback=progress_callback)
    ...
    column_download_layout = [[sg.Text("Downloading Part", font="Arial 16 bold underline", text_color="#c63a3d")],
                              ...
                              [sg.Text("Enter URL:"), sg.Input(key="-LINK_INPUT-", size=(31, 1), right_click_menu=MENU_RIGHT_CLICK_COPY_PASTE), sg.Button("+", size=(3, 1), key="-APPEND_URL_TO_LIST-"), sg.Button("Download", key="-BUTTON_DOWNLOAD-"), sg.Button("Stream Info", key="-STREAM_INFO_BUTTON-")]]

    column_output_and_progressbar = [[sg.Listbox(values=[], size=(65, 6), key='-URL_LIST-', right_click_menu=MENU_RIGHT_CLICK_DELETE)],
                                     [sg.HSeparator()],
                                     [sg.Multiline(size=(65, 8), key="-OUTPUT_WINDOW-")],
    ...
    # ====== GUI Events ======#
    window = sg.Window("YouTube Downloader | Credit: Jovan Zaric", layout,font="Arial 16", finalize=True, right_click_menu=MENU_RIGHT_CLICK)

    # Variables
    urls = []  # List to store YouTube URLs
    listbox_urls = [] # To display in the listbox (test, delete after?)
    ...
    while True:
        ...
        # Closing the programm with either option [X]
        ...
        if event in ('Copy', 'Paste'):
            try:
                ...
                window["-OUTPUT_WINDOW-"].update("ERROR: Something went wrong!")
        ...
        # Append URLS to list and updating PySimpleGUI's ListBox
            ...
            if "https://www.youtube.com/watch" not in url:
                ...
                urls.append(url)
                listbox_urls.append(f"{id_counter}. {yt.title}")
                id_counter += 1
        ...
        # Downloading Video File
            ...
            if len(values["-SAVE_TO_FOLDER-"]) == 0:
                window["-OUTPUT_WINDOW-"].update("ERROR: Please enter a path where to save the video download.")
            elif len(urls) == 0:
                window["-OUTPUT_WINDOW-"].update("ERROR. You haven't added any links for downloading, add a link to the input and press the + button.")
            else:
                window.perform_long_operation(lambda: download_youtube_video(urls, values["-SAVE_TO_FOLDER-"], values["-QUALITY_FORMAT-"]), "-OUTPUT_WINDOW-")

        # Downloading Audio File
        elif event == "-BUTTON_DOWNLOAD-" and values["-DOWNLOAD_FORMAT-"] == "Audio Format":

            if len(values["-SAVE_TO_FOLDER-"]) == 0:
                window["-OUTPUT_WINDOW-"].update("ERROR: Please enter a path where to save the audio download.")
            elif len(urls) == 0:
                window["-OUTPUT_WINDOW-"].update("ERROR. You haven't added any links for downloading, add a link to the input and press the + button.")
            else:
                window.perform_long_operation(lambda: download_youtube_audio(urls, values["-SAVE_TO_FOLDER-"], values["-QUALITY_FORMAT-"]), "-OUTPUT_WINDOW-")

        # Get Stream Info of YouTube Link for Video Downloading
        if event == "-STREAM_INFO_BUTTON-" and values["-DOWNLOAD_FORMAT-"] == "Video Format":
            if len(urls) != 0:
                get_video_stream_thread = threading.Thread(target=video_stream, args=(urls[0],))
                get_video_stream_thread.start()
            else:
                window["-OUTPUT_WINDOW-"].update("ERROR: Listbox is empty, add links to it and try again.")

         # Get Stream Info of YouTube Link for Audio Downloading
        if event == "-STREAM_INFO_BUTTON-" and values["-DOWNLOAD_FORMAT-"] == "Audio Format":
            if len(urls) != 0:
                get_audio_stream_thread = threading.Thread(target=audio_stream, args=(urls[0],))
                get_audio_stream_thread.start()
            else:
                window["-OUTPUT_WINDOW-"].update("ERROR: Listbox is empty, add links to it and try again.")
                
        # Direct Link redirect for FFmpeg for Windows
